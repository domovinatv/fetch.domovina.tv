...
- **⚠️ NIZAK CONCURRENCY na USB diskovima** (empirijski 2026-05-29): mp4 mux je I/O-bound (čita+piše velike video bajtove). Visok concurrency (6) na eksternim USB diskovima izazove **I/O thrashing** — sekvencijalna čitanja/pisanja postanu random, throughput kolabira (test: 6 paralelnih = 51 min, 0 gotovih, `df` visi). Koristi **`--concurrency 2`** (možda i 1). Manje je BRŽE jer nema thrashinga. (Isti razlog je mp3 backfill na C=6 trajao ~13 h.) Bez `--verify` za backfill — prepolovi I/O (loudnorm summary u sidecaru ionako daje output LUFS/TP).
